...
public class NFPBFNFPBreastFeedingAPIClient: AWSAPIGatewayClient {

	static let AWSInfoClientKey = "NFPBFNFPBreastFeedingAPIClient"

	private static let _serviceClients = AWSSynchronizedMutableDictionary()
	private static let _defaultClient:NFPBFNFPBreastFeedingAPIClient = {
        ...
        return NFPBFNFPBreastFeedingAPIClient(configuration: serviceConfiguration!)
	}()
	...
	/**
	 ...
	 Then call the following to get the default service client:
	     ...
	     let serviceClient = NFPBFNFPBreastFeedingAPIClient.default()

     Alternatively, this configuration could also be set in the `info.plist` file of your app under `AWS` dictionary with a configuration dictionary by name `NFPBFNFPBreastFeedingAPIClient`.
	
	 @return The default service client.
	 */ 
	 
	public class func `default`() -> NFPBFNFPBreastFeedingAPIClient{
	...
	/**
	 ...
	 If you want to enable AWS Signature, set the default service configuration in `func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?)`
	     ...
	     func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
	         ...
	         NFPBFNFPBreastFeedingAPIClient.registerClient(withConfiguration: configuration, forKey: "USWest2NFPBFNFPBreastFeedingAPIClient")
	 ...
	 Then call the following to get the service client:
	     ...
	     let serviceClient = NFPBFNFPBreastFeedingAPIClient.client(forKey: "USWest2NFPBFNFPBreastFeedingAPIClient")
	...
	public class func registerClient(withConfiguration configuration: AWSServiceConfiguration, forKey key: String){
		_serviceClients.setObject(NFPBFNFPBreastFeedingAPIClient(configuration: configuration), forKey: key  as NSString);
	}
	 ...
	 */
	public class func client(forKey key: String) -> NFPBFNFPBreastFeedingAPIClient {
		objc_sync_enter(self)
		if let client: NFPBFNFPBreastFeedingAPIClient = _serviceClients.object(forKey: key) as? NFPBFNFPBreastFeedingAPIClient {
			objc_sync_exit(self)
		    return client
		}
		...
		if let serviceInfo = serviceInfo {
			...
			NFPBFNFPBreastFeedingAPIClient.registerClient(withConfiguration: serviceConfiguration!, forKey: key)
		}
		objc_sync_exit(self)
		return _serviceClients.object(forKey: key) as! NFPBFNFPBreastFeedingAPIClient;
	}
    ...
    /*
     ...
     return type: 
     */
    public func cognitoDisableuserPost(body: NFPBFCognitouser) -> AWSTask<AnyObject> {
